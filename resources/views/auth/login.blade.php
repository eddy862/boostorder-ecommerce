<x-guest-layout title="Login">
    <div class="mb-6">
        <p class="text-sm font-semibold uppercase tracking-[0.22em] text-orange-400">{{ __('Welcome back') }}</p>
        <h2 class="mt-2 text-3xl font-black text-gray-800">{{ __('Log in to your account') }}</h2>
        <p class="mt-2 text-sm leading-6 text-gray-500">{{ __('Continue shopping, review your cart, and manage your recent orders.') }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="mt-5 flex items-center justify-between gap-3">
            @if (Route::has('password.request'))
                <a class="text-sm font-medium text-orange-500 hover:text-orange-600 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-400" href="{{ route('password.request') }}">
        ...
        <div class="flex items-center justify-center mt-4">
            ...
            <a class="ms-3 text-sm font-semibold text-orange-500 hover:text-orange-600 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-400" href="{{ route('register') }}">
                {{ __('Register') }}
            </a>
        </div>
    </form>
